...
void extract_features(const float* accY, const float* accZ, const float* pressure, const float* gyroY, const float* gyroZ, size_t length, Features_t* features) {
    ...
    // max
    float y_max = accY[0], pressure_max = pressure[0];
    for (size_t i = 1; i < length; i++) {
        if (accY[i] > y_max) y_max = accY[i];
        if (pressure[i] > pressure_max) pressure_max = pressure[i];
    }
    features->y_max = y_max;
    features->pressure_max = pressure_max;
